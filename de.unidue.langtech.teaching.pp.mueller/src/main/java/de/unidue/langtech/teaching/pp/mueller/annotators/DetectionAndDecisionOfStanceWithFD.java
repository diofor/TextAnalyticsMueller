...
public class DetectionAndDecisionOfStanceWithFD extends JCasAnnotator_ImplBase
{
	...
	public void process(JCas aJCas) throws AnalysisEngineProcessException {
		...
		String haeufigstesStance = "";
		long countHaeufigstesStance = Integer.MIN_VALUE;
		for(String key : fd_tweet.getKeys())
		{
			if (fd_tweet.getCount(key)> countHaeufigstesStance)
			{
				haeufigstesStance = key;
				countHaeufigstesStance = fd_tweet.getCount(key);
			}
		}
		
		DetectedInformation di = JCasUtil.selectSingle(aJCas, DetectedInformation.class);
		di.setStance(haeufigstesStance);
		di.addToIndexes();
